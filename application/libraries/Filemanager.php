...
require_once(MTFM_PATH . '/Filemanager/Upload.php');
require_once(MTFM_PATH . '/Filemanager/Image.php');
...
//      'directory' => MTFM_PATH . '/files/',
      ...
      'thumbnailPath' => MTFM_PATH . '/files/.thumbs',  // written like this so we're completely clear on where the default thumbnails directory will be
      ...
      /*
       * id3Path : Added by Partikule : The getid3 lib can already exists, be used but stored at another place in the website dir
       *
       */
      'id3Path' => MTFM_PATH . '/getid3/getid3.php',
      /*
       * Default thumb size : Added by Partikule : As option, the user can define the thumbs sizes.
       *
       */
	  'thumbnailSize' => 120
  ...
  private function _onView($dir, $json, $mime_filter, $list_type)
  {
    ...
    foreach ($files as $file)
    {
	  ...
	  if ($mime_filter && $mime != 'text/directory' && !$this->IsAllowedMimeType($mime, $mime_filters))
		continue;
...
// Partikule
// The icon is always the file type icon, the thumb is the thumb, not an icon.
// The thumbnail can be the picture thumbnail in case of picture or the large icon in case of file.
...
//        $mime_filter = ((isset($_POST['filter']) && !empty($_POST['filter'])) ? $_POST['filter'].'/' : null);
        $mime_filter = ( ! empty($_POST['filter']) ? $_POST['filter'].'/' : $this->options['filter']);
...
// Partikule
// The user know this is a preview...
//					        <h2>${preview}</h2>';
...
//					$tnc = '<a href="'.$encoded_url.'" data-milkbox="preview" title="'.htmlentities($file, ENT_QUOTES, 'UTF-8').'"><img src="' . FileManagerUtility::rawurlencode_path($thumbfile) . $randomImage . '" class="preview" alt="preview" /></a>';
					$tnc = '<a href="'.$encoded_url.'" data-milkbox="preview" title="'.htmlentities($file, ENT_QUOTES, 'UTF-8').'"><img src="' . $url . $randomImage . '" class="preview" alt="preview" /></a>';
				}
				catch (Exception $e)
				{
...
// Partikule
// Removed the title 'preview' : More vertical space and the user see himself it's a preview.
			$content = '<dl>
...
// Partikule
// Added the silent getid3 : Some MP3 can have internal tags not set.
// Get of the html id3v2 tags : Some special char aren't well rendered in id3v1 indexes. Example : "Nawâr"
// Removed the title 'preview' : More vertical space and the user see himself it's a preview.
				$content = '<dl>
...
// Added the FLV support
			elseif (FileManagerUtility::startsWith($mime, 'video/x-flv'))
			{
				...
				$content = '
					<div class="object">
						<object type="application/x-shockwave-flash" data="' . $this->options['baseURL'] . 'themes/admin/flash/mediaplayer/player.swf?file=' . FileManagerUtility::rawurlencode_path($url) . '" width="170" height="145">
							<param name="wmode" value="transparent" />
							<param name="movie" value="' . $this->options['baseURL'] . 'themes/admin/flash/mediaplayer/player.swf?file=' . FileManagerUtility::rawurlencode_path($url) . '" />
						</object>
					</div>
  ...
  protected function onCreate()
  {
    try
    {
        $mime_filter = ((isset($_POST['filter']) && !empty($_POST['filter'])) ? $_POST['filter'].'/' : $this->options['filter']);
        $list_type = ((isset($_POST['type']) && $_POST['type'] == 'list') ? 'list' : 'thumb');
